...
class LoadWasm {
    ...
    async load(tasks) {
        ...
        let response
        try {
            response = await fetch(one_path)
            if (!response.ok) {
                util.handle_wasm_error('HTTP error!', one_path, `${response.status}`)
                return
            }
        } catch (error) {
            util.handle_wasm_error('Error fetching WASM module', one_path, error)
            return
        }
        ...
        try {
            ...
            util.handle_wasm_error('Error converting response to ArrayBuffer for', one_path, error)
            return
        }

        util.compile(bytes, one_path)
    }
}


export var sm_hub = new Smh()
